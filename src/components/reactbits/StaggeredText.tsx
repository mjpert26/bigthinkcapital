...
const StaggeredText = forwardRef<StaggeredTextHandle, StaggeredTextProps>(
  ({ text, className = "", as = "p", segmentBy = "words", separator, delay = 80, duration = 0.6,
    easing = (t: number) => t, threshold = 0.1, rootMargin = "0px", direction = "top", blur = true,
    from, to, staggerDirection = "forward", respectReducedMotion = true, exitOnScrollOut = false,
    onAnimationComplete, onExitComplete }, ref) => {
    ...
    useImperativeHandle(ref, () => ({
      replay: () => { setIsExiting(false); setHasEnteredView(false); requestAnimationFrame(() => setHasEnteredView(true)); },
    ...
    useEffect(() => {
      ...
      const observer = new IntersectionObserver(([entry]) => {
        if (entry.isIntersecting) { setHasEnteredView(true); setIsExiting(false); if (!exitOnScrollOut && rootRef.current) observer.unobserve(rootRef.current); }
        else if (exitOnScrollOut && hasEnteredView) setIsExiting(true);
    ...
    const defaultFrom = useMemo<MotionStyle>(() => {
      ...
      if (direction === "top" || direction === "bottom") base.y = direction === "top" ? -40 : 40;
      else base.x = direction === "left" ? -40 : 40;
    ...
    const defaultTo = useMemo<MotionStyle[]>(() => {
      ...
      return [{ opacity: 0.7, [axisKey]: overshoot, filter: "blur(5px)" }, { opacity: 1, [axisKey]: 0, filter: "blur(0px)" }];
    }, [direction, blur]);

    const fromSnapshot = useMemo<MotionStyle>(() => from ?? defaultFrom, [from, defaultFrom]);
    const toSnapshots = useMemo<MotionStyle[]>(() => { if (!to) return defaultTo; return Array.isArray(to) ? to : [to]; }, [to, defaultTo]);
    ...
    const getStaggerDelay = (index: number, total: number): number => {
      ...
      switch (staggerDirection) {
        ...
        case "center": { const middle = (total - 1) / 2; return (Math.abs(index - middle) * delay) / 1000; }
    ...
    return (
      <Wrapper ref={rootRef as React.RefObject<HTMLParagraphElement>}
        ...
        {rowsSegments.map((rowSegments, rowIndex) => (
          <React.Fragment key={`row-${rowIndex}`}>
            {rowSegments.map((segment, rowSegIndex) => {
              ...
              return (
                <motion.span key={`seg-${rowIndex}-${rowSegIndex}`} initial={fromSnapshot}
                  animate={isExiting ? fromSnapshot : hasEnteredView ? animateKeyframes : fromSnapshot}
                  transition={transition}
                  onAnimationComplete={isLast ? () => { if (isExiting) onExitComplete?.(); else onAnimationComplete?.(); } : undefined}
